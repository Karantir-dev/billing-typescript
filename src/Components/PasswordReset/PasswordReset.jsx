...
export function PasswordReset() {
  ...
  //             <button className={s.submit_btn} type="submit" disabled={!email}>
    ...
    <div className={s.form_wrapper}>
      ...
      <form className={s.form} onSubmit={handleSubmit}>
        ...
        <div className={s.field_wrapper}>
          ...
          {valid === 'invalid' && (
            <span className={s.error_message}>{t('warnings.wrong_email')}</span>
          )}
        </div>
        <button className={s.submit_btn} type="submit" onClick={handleSubmit}>
          <span className={s.btn_text}>{t('reset.resetButton')}</span>
        </button>
        <Link className={s.reset_pass_link} to={routes.LOGIN}>
          {t('reset.resetCancel')}
